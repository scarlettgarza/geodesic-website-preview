...
const artifactSurface = 'var(--artifact-paper)'
const artifactSurfaceAlt = 'var(--artifact-paper-2)'
const artifactSurfaceMuted = 'var(--artifact-paper-muted)'
const artifactInk = 'var(--artifact-ink)'
const artifactInkSoft = 'var(--artifact-ink-soft)'
const artifactLabel = 'var(--artifact-label)'
const artifactRule = 'var(--artifact-rule)'
const artifactRuleSoft = 'var(--artifact-rule-2)'
const artifactRuleStrong = 'var(--artifact-rule-strong)'
const artifactAlert = 'var(--artifact-alert)'
const artifactAlertAlt = 'var(--artifact-alert-2)'
const artifactPositive = 'var(--artifact-positive)'
const artifactNote = 'var(--artifact-note)'
const artifactQuickBooks = 'var(--artifact-qb)'
const artifactQuickBooksBorder = 'var(--artifact-qb-border)'
const artifactSlack = 'var(--artifact-slack)'
const artifactWarningPaper = 'var(--artifact-warning-paper)'
const artifactWarningBar = 'var(--artifact-warning-bar)'
const artifactWarningText = 'var(--artifact-warning-text)'
const artifactCrm = 'var(--artifact-crm)'
const artifactCrmBar = 'var(--artifact-crm-bar)'
  ...
  return (
    <div
      style={{
        ...
        height: 225,
        borderRadius: 2,
        background: `${grainOverlay}, linear-gradient(90deg, var(--artifact-paper) 0%, var(--artifact-paper) 100%)`,
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 7.5 }}>
        ...
        <span style={{ fontSize: 11.3, fontWeight: 600, color: 'var(--artifact-label)' }}>PG 14</span>
      </div>
      <div style={{ height: 1, background: 'var(--artifact-rule)', marginBottom: 13.5 }} />
      <div style={{ fontSize: 15.8, lineHeight: 1.7 }}>
        ...
        <div style={{ fontStyle: 'italic', color: 'var(--artifact-ink-soft)', marginTop: 3 }}>&quot;Executing well against plan&quot;</div>
      </div>
    </div>
  )
}
...
function VendorContractsCard() {
  ...
  const cellStyle = { padding: '6px 9px', borderBottom: '1px solid var(--artifact-rule-2)', fontSize: 14.6 }
  const headerStyle = { ...cellStyle, fontWeight: 700, fontSize: 11.3, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--artifact-label)', borderBottom: '2px solid var(--artifact-rule-strong)' }
  ...
  return (
    <div
      style={{
        ...
        height: 203,
        borderRadius: 2,
        background: `${grainOverlay}, var(--artifact-paper-2)`,
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>
              ...
              <td style={{ ...cellStyle, color: r.end === '???' ? 'var(--artifact-alert)' : undefined, fontWeight: r.end === '???' ? 700 : 400 }}>{r.end}</td>
              <td style={{ ...cellStyle, color: 'var(--artifact-alert)' }}>{r.keyPerson}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div
      style={{
        ...
        height: 180,
        borderRadius: 2,
        background: `${darkGrainOverlay}, var(--artifact-warning-paper)`,
    ...
    >
      <div
        style={{
          height: 35,
          background: 'var(--artifact-warning-bar)',
          display: 'flex',
          alignItems: 'center',
          padding: '0 18px',
        }}
      >
        <span style={{ fontSize: 11.3, fontWeight: 600, color: 'var(--ivory)', textTransform: 'uppercase', letterSpacing: '1px' }}>
          FROM: FORMER CFO &rarr; BOARD (7 MONTHS AGO)
        </span>
      </div>
      <div style={{ padding: '15px 18px', color: 'var(--artifact-warning-text)', fontSize: 15.8, fontStyle: 'italic', lineHeight: 1.6 }}>
  ...
  return (
    <div
      style={{
        ...
        height: 225,
        borderRadius: 2,
        background: `${darkGrainOverlay}, var(--artifact-crm)`,
        boxShadow: paperShadow,
        fontFamily: mono,
        overflow: 'hidden',
        display: 'flex',
    ...
    >
      <div
        style={{
          height: 38,
          background: 'var(--artifact-crm-bar)',
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <span style={{ fontSize: 18, fontWeight: 700, color: 'var(--ivory)' }}>CRM EXPORT</span>
        <span style={{ fontSize: 11.3, fontWeight: 700, color: 'var(--artifact-alert)' }}>RAW</span>
      </div>
      <div style={{ padding: '15px 18px', color: 'var(--artifact-ink)', fontSize: 15.8, lineHeight: 1.7 }}>
        <div>Actual retention: <span style={{ color: 'var(--artifact-alert)', fontWeight: 700 }}>81%</span></div>
        <div>Pipeline: 47% &quot;verbal commit&quot; &mdash;</div>
        <div><span style={{ textDecoration: 'line-through', color: 'var(--artifact-alert)' }}>no signed LOI</span></div>
      </div>
    </div>
  )
}
  ...
  return (
    <div
      style={{
        ...
        height: 180,
        borderRadius: 4,
        background: `${grainOverlay}, var(--artifact-note)`,
  ...
  return (
    <div
      style={{
        ...
        height: 203,
        borderRadius: 2,
        background: 'var(--artifact-paper-muted)',
        boxShadow: tableShadow,
        padding: '21px 21px',
        fontFamily: mono,
        color: 'var(--artifact-ink)',
    ...
    >
      <div>CEO (interim)</div>
      <div>├─ <span style={{ textDecoration: 'line-through', color: 'var(--artifact-alert)' }}>CFO — terminated</span></div>
  ...
  return (
    <div
      style={{
        ...
        height: 180,
        borderRadius: 2,
        background: 'linear-gradient(180deg, var(--artifact-paper) 0%, var(--artifact-paper-2) 100%)',
        boxShadow: tableShadow,
        padding: '18px 21px',
        fontFamily: mono,
        color: 'var(--artifact-ink)',
    ...
    >
      ...
      <div style={{ fontSize: 15.8, lineHeight: 1.7 }}>
        <div>Account: Top 10 = <span style={{ color: 'var(--artifact-positive)', fontWeight: 700 }}>72%</span> ARR</div>
        <div>Pipeline: $4M reported</div>
        <div style={{ color: 'var(--artifact-alert-2)' }}>12 opps last updated 90+ days ago</div>
      </div>
    </div>
  )
}
...
function ToolCardZendesk() {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 15.8, lineHeight: 1.7 }}>
        <div style={{ color: 'var(--artifact-alert-2)' }}>Support Volume &uarr;40% QoQ</div>
  ...
  return (
    <div
      style={{
        ...
        position: 'relative',
        overflow: 'hidden',
        borderLeft: '6px solid var(--artifact-qb-border)',
      }}
    >
      <img
      ...
      <div style={{ fontSize: 15.8, lineHeight: 1.7 }}>
        <div style={{ color: 'var(--artifact-alert-2)' }}>Sprint velocity &darr;60% QoQ</div>
...
function ToolCardNetSuite() {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 15.8, lineHeight: 1.7 }}>
        <div style={{ color: 'var(--artifact-alert-2)' }}>GL close: 23 days overdue</div>
        <div style={{ color: 'var(--artifact-alert-2)' }}>Intercompany: 4 unreconciled</div>
  ...
  return (
    <div
      style={{
        ...
        height: 169,
        borderRadius: 2,
        background: `${grainOverlay}, var(--artifact-qb)`,
        boxShadow: tableShadow,
        padding: '18px 21px',
        fontFamily: mono,
        color: 'var(--artifact-ink)',
  ...
  return (
    <div
      style={{
        ...
        height: 158,
        borderRadius: 4,
        background: `${grainOverlay}, var(--artifact-slack)`,
        boxShadow: tableShadow,
        padding: '18px 21px',
        fontFamily: mono,
        color: 'var(--artifact-ink)',
    ...
    >
      ...
      <div style={{ fontSize: 15.8, fontStyle: 'italic', lineHeight: 1.6, color: 'var(--artifact-ink-soft)' }}>
...
function ConnectionLine({ x1, y1, x2, y2, scrollYProgress }) {
  ...
  return (
    <motion.line x1={x1} y1={y1} x2={x2} y2={y2}
      stroke="var(--accent-secondary)" strokeWidth={1.5}
      strokeDasharray={len}
...
function ConnectionDot({ cx, cy, scrollYProgress }) {
  ...
  return <motion.circle cx={cx} cy={cy} r={4} fill="var(--accent-secondary)" style={{ opacity }} />
}
...
function InvestigationVisual() {
  return (
    <div style={{ backgroundColor: 'var(--viz-shell-bg)', borderRadius: 8, padding: '20px 24px', color: 'var(--viz-shell-text)' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 11, color: 'var(--viz-shell-label)', textTransform: 'uppercase', letterSpacing: '0.5px', paddingBottom: 8, borderBottom: '1px solid var(--viz-shell-border)' }}>
  ...
  return (
    <section ref={sectionRef} style={{ height: '700vh', position: 'relative' }}>
      <motion.div style={{
        ...
        overflow: 'hidden', backgroundColor: 'var(--page-bg)',
      }}>
        {/* Cards */}
        {CARDS.map((card, i) => {
        ...
        <motion.div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 40, position: 'relative', flexShrink: 0 }}>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: 'clamp(3rem, 6vw, 5.6rem)', lineHeight: 1.1, color: 'var(--text-primary)' }}>
                <motion.span style={{ opacity: hl1Opacity, display: 'block' }}>
                  <span style={{ color: 'var(--accent-primary)' }}>Building the picture.</span>{' '}
                ...
                <motion.span style={{ opacity: hl2Opacity, position: 'absolute', inset: 0 }}>
                  ...
                  <span style={{ color: 'var(--accent-primary)' }}>Doing the work.</span>{' '}
                ...
                <motion.span style={{ opacity: hl3Opacity, position: 'absolute', inset: 0 }}>
                  ...
                  <span style={{ color: 'var(--accent-primary)' }}>Owning the outcome.</span>
                </motion.span>
              </h2>
            </div>

            {/* Content grid: text left, visual right — vertically centered in a shared container */}
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(320px, 0.9fr) minmax(420px, 1fr)', gap: 64, alignItems: 'center', flexShrink: 0 }}>
              {/* Left column: subhead + body */}
              <div style={{ position: 'relative', minHeight: 180, display: 'flex', alignItems: 'center' }}>
              {/* Panel 1 text */}
              ...
              {/* Panel 2 visual: investigation findings */}
                <motion.div style={{ opacity: p2Opacity, position: 'absolute', inset: 0, maxHeight: '100%', overflow: 'auto', display: 'flex', alignItems: 'center' }}>
                <InvestigationVisual />
              </motion.div>
              {/* Panel 3 visual: evidence view */}
                <motion.div style={{ opacity: p3Opacity, position: 'absolute', inset: 0, maxHeight: '100%', overflow: 'auto', display: 'flex', alignItems: 'center' }}>
                <EvidenceVisual />
                </motion.div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 28, flexShrink: 0 }}>
              <motion.div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--accent-primary)', opacity: dot1Active }} />
              <motion.div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--accent-primary)', opacity: dot2Active }} />
              <motion.div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--accent-primary)', opacity: dot3Active }} />
            </div>
          </div>
        </motion.div>
      </motion.div>
