...
const connectedCards = [
  {
    ...
    content: (
      <div className="flex items-end gap-1.5 mt-2 h-10">
        <div style={{ width: 14, height: '70%', backgroundColor: 'var(--viz-positive)', borderRadius: 2 }} />
        <div style={{ width: 14, height: '45%', backgroundColor: 'var(--viz-positive-soft)', borderRadius: 2 }} />
        <div style={{ width: 14, height: '90%', backgroundColor: 'var(--viz-positive)', borderRadius: 2 }} />
        <div style={{ width: 14, height: '55%', backgroundColor: 'var(--viz-muted)', borderRadius: 2 }} />
        <div style={{ width: 14, height: '30%', backgroundColor: 'var(--viz-positive-soft)', borderRadius: 2 }} />
      </div>
    ),
  },
  {
    ...
    content: (
      <div className="mt-2 space-y-1">
        {['Active — 42', 'Renewing — 8', 'At Risk — 3'].map((row) => (
          <div key={row} className="flex justify-between text-[10px]" style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>
  ...
  },
  {
    ...
    content: (
      <div className="mt-2 space-y-1.5">
        {[
          { label: 'Q1', pct: 82, color: 'var(--viz-positive)' },
          { label: 'Q2', pct: 64, color: 'var(--viz-positive-soft)' },
          { label: 'Q3', pct: 38, color: 'var(--viz-muted)' },
        ].map((bar) => (
          <div key={bar.label} className="flex items-center gap-2">
            <span className="text-[9px] w-5" style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{bar.label}</span>
            <div style={{ flex: 1, height: 6, backgroundColor: 'var(--viz-grid)', borderRadius: 2 }}>
  ...
  },
  {
    ...
    content: (
      <div className="mt-2 flex flex-col items-center">
        <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: 'var(--viz-positive)' }} />
        <div style={{ width: 1, height: 8, backgroundColor: 'var(--viz-warning)' }} />
        <div className="flex gap-6">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex flex-col items-center">
              <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--viz-positive-soft)' }} />
              <div style={{ width: 1, height: 6, backgroundColor: 'var(--viz-warning)' }} />
              <div className="flex gap-1">
                <div style={{ width: 5, height: 5, borderRadius: '50%', backgroundColor: 'var(--viz-muted)' }} />
                <div style={{ width: 5, height: 5, borderRadius: '50%', backgroundColor: 'var(--viz-muted)' }} />
              </div>
            </div>
          ))}
    ...
    content: (
      <div className="mt-2 flex items-end gap-0.5 h-8">
        {[3, 5, 4, 7, 6, 8, 7, 9, 8, 10, 9].map((v, i) => (
          <div key={i} style={{ width: 6, height: `${v * 10}%`, backgroundColor: i > 7 ? 'var(--viz-positive)' : 'var(--viz-positive-soft)', borderRadius: 1 }} />
        ))}
      </div>
    ),
  },
    ...
    content: (
      <div className="flex items-end gap-1.5 mt-2 h-10">
        {[60, 45, 70, 50, 80, 65].map((h, i) => (
          <div key={i} style={{ width: 12, height: `${h}%`, backgroundColor: i >= 4 ? 'var(--viz-positive)' : 'var(--viz-positive-soft)', borderRadius: 2 }} />
        ))}
      </div>
    ),
  },
    ...
    content: (
      <div className="mt-2 space-y-1.5">
        {[
          ...
          <div key={p.name} className="flex items-center gap-2 text-[10px]" style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)' }}>
            <div style={{
              width: 7, height: 7, borderRadius: '50%',
              backgroundColor: p.risk === 'high' ? 'var(--viz-danger)' : p.risk === 'med' ? 'var(--viz-warning)' : 'var(--viz-positive-soft)',
            }} />
            <span>{p.name}</span>
          </div>
        ))}
    ...
    content: (
      <div className="mt-2 space-y-1">
        {['Rev  $12.4M', 'Burn  $1.8M', 'Runway  14mo'].map((row) => (
          <div key={row} className="text-[10px]" style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>
            {row}
          </div>
        ))}
      </div>
            ...
            >
              <div
                style={{
                  width: '100%',
                  height: '100%',
                  backgroundColor: 'var(--artifact-paper)',
                  borderRadius: 3,
                  padding: '10px 12px',
  ...
  return (
    <div className="relative">
      <motion.span style={{ opacity: panel0Opacity }} className="block">
        ...
        <span style={{ color: 'var(--text-secondary)' }}>Doing the work.</span>{' '}
        <span style={{ color: 'var(--text-secondary)' }}>Owning the outcome.</span>
      </motion.span>
      <motion.span style={{ opacity: panel1Opacity }} className="absolute inset-0">
        <span style={{ color: 'var(--text-secondary)' }}>Building the picture.</span>{' '}
  ...
  return (
    <div className="relative min-h-[200px]">
      {panels.map((panel, i) => (
        ...
        >
          <h3 style={{ fontFamily: 'var(--font-serif)', fontWeight: 500, color: 'var(--text-primary)', fontSize: '1.25rem', marginBottom: '1rem' }}>
            {panel.subhead}
          </h3>
